	...
	for(var i=0;i<oEditor.document.all.length;i++)
		...
		if(oEditor.document.all[i].tagName=="A")
			...
			if(oEditor.document.all[i].name!="")
				oEditor.document.all[i].innerHTML = "<IMG src='library/ACE/include/yusasp_anchor.gif' align=middle border=0>" + oEditor.document.all[i].innerHTML;
			}
		}
	//~~~~~~~~~~~~~
	}
...
function RUN()
	...
	if(this.useSave)obj.drawIcon(1,0,27,"btnSave"+oName,"Opslaan",oName+".onSave()",false);
	...
	if(this.useStyle)obj.drawIcon(1,156,58,"btnStyle"+oName,"Stijl",oName+".boxShowDrop('boxStyle"+oName+"');setPosition(this,'boxStyle"+oName+"')",false);
	
	if(this.useParagraph)obj.drawIcon(2,0,75,"btnParagraph"+oName,"Alinea",oName+".boxShowDrop('boxParagraph');setPosition(this,'boxParagraph')",false);
	if(this.useFontName)obj.drawIcon(2,52,75,"btnFontName"+oName,"Lettertype",oName+".boxShowDrop('boxFont');setPosition(this,'boxFont')",false);
	if(this.useSize)obj.drawIcon(2,104,55,"btnSize"+oName,"Lettergrootte",oName+".boxShowDrop('boxSize');setPosition(this,'boxSize')",false);
	if(this.useText)obj.drawIcon(2,156,27,"btnText"+oName,"Tekst",oName+".InsertText()",false);

	if(this.useSelectAll)obj.drawIcon(3,0,27,"btnSelectAll"+oName,"Alles selecteren",oName+".doCmd('SelectAll')",false);
	if(this.useCut)obj.drawIcon(3,52,27,"btnCut"+oName,"Knippen",oName+".doCmd('Cut')",false);
	if(this.useCopy)obj.drawIcon(3,104,27,"btnCopy"+oName,"Kopieren",oName+".doCmd('Copy')",false);
	if(this.usePaste)obj.drawIcon(3,156,27,"btnPaste"+oName,"Plakken",oName+".doCmd('Paste')",false);
	
	if(this.useUndo)obj.drawIcon(4,0,27,"btnUndo"+oName,"Ongedaan maken",oName+".doCmd('Undo')",false);
	if(this.useRedo)obj.drawIcon(4,52,27,"btnRedo"+oName,"Opnieuw doen",oName+".doCmd('Redo')",false);
	if(this.useBold)obj.drawIcon(4,104,27,"btnBold"+oName,"Vet",oName+".doCmd('Bold')",false);
	if(this.useItalic)obj.drawIcon(4,156,27,"btnItalic"+oName,"Schuin",oName+".doCmd('Italic')",false);

	if(this.useUnderline)obj.drawIcon(5,0,27,"btnUnderline"+oName,"Onderstreept",oName+".doCmd('Underline')",false);
	if(this.useStrikethrough)obj.drawIcon(5,52,27,"btnStrikethrough"+oName,"Doorgestreept",oName+".doCmd('Strikethrough')",false);
	...
	if(this.useSymbol)obj.drawIcon(6,0,27,"btnSymbol"+oName,"Symbool invoegen",oName+".boxShowPop('boxSymbol')",false);
	if(this.useJustifyLeft)obj.drawIcon(6,52,27,"btnJustifyLeft"+oName,"Links uitlijnen",oName+".doCmd('JustifyLeft')",false);
	if(this.useJustifyCenter)obj.drawIcon(6,104,27,"btnJustifyCenter"+oName,"Centreren",oName+".doCmd('JustifyCenter')",false);
	if(this.useJustifyRight)obj.drawIcon(6,156,27,"btnJustifyRight"+oName,"Rechts uitlijnen",oName+".doCmd('JustifyRight')",false);

	if(this.useJustifyFull)obj.drawIcon(7,0,27,"btnJustifyFull"+oName,"Uitvullen",oName+".doCmd('JustifyFull')",false);
	if(this.useNumbering)obj.drawIcon(7,52,27,"btnNumbering"+oName,"Nummering",oName+".doCmd('InsertOrderedList')",false);
	if(this.useBullets)obj.drawIcon(7,104,27,"btnBullets"+oName,"Opsomming",oName+".doCmd('InsertUnorderedList')",false);
	if(this.useIndent)obj.drawIcon(7,156,27,"btnIndent"+oName,"Inspringing vergroten",oName+".doCmd('Indent')",false);
	
	if(this.useOutdent)obj.drawIcon(8,0,27,"btnOutdent"+oName,"Inspringing verkleinen",oName+".doCmd('Outdent')",false);
	if(this.useImage)obj.drawIcon(8,52,27,"btnImage"+oName,"Afbeelding invoegen",oName+".boxShow_Image()",false);
	if(this.useForeColor)obj.drawIcon(8,104,27,"btnForeColor"+oName,"Tekstkleur",oName+".boxShowPop('boxForecolor')",false);
	if(this.useBackColor)obj.drawIcon(8,156,27,"btnBackColor"+oName,"Achtergrondkleur",oName+".boxShowPop('boxBackcolor')",false);

	if(this.useExternalLink)obj.drawIcon(9,0,27,"btnExternalLink"+oName,"Externe Link",oName+".boxShow_ExternalLink()",false);
	if(this.useInternalLink)obj.drawIcon(9,52,27,"btnInternalLink"+oName,"Interne Link",oName+".boxShow_InternalLink()",false);
	if(this.useAsset)obj.drawIcon(9,104,27,"btnAsset"+oName,"Bestand invoegen",oName+".boxShow_Asset()",false);
	if(this.useTable)obj.drawIcon(9,156,27,"btnTable"+oName,"Tabel menu",oName+".boxShow_Table()",false);
	
	if(this.useShowBorder)obj.drawIcon(10,0,27,"btnShowBorder"+oName,"Maak randen zichtbaar",oName+".showBorder()",false);
	if(this.useAbsolute)obj.drawIcon(10,52,27,"btnAbsolute"+oName,"Absolute positie",oName+".doCmd('AbsolutePosition')",false);//IE5.5 or later
	if(this.useClean)obj.drawIcon(10,104,27,"btnClean"+oName,"Verwijder opmaak",oName+".doCmd('RemoveFormat')",false);
	if(this.useLine)obj.drawIcon(10,156,27,"btnLine"+oName,"Horizontale lijn",oName+".doCmd('InsertHorizontalRule')",false);
	...
	document.write("	<input type=checkbox onclick='"+oName+".setDisplayMode()' id=chkDisplayMode name=chkDisplayMode><font face=verdana size=1> HTML VIEW &nbsp;</font>");
	document.write("	<input type='checkbox' id=chkBreakMode"+oName+" name=chkBreakMode"+oName+"><font face=verdana size=1> ENTER TOETS 1 regel naar beneden &nbsp;</font>");
	...
	if(this.useForeColor && !document.getElementById("boxForecolor"))//boxForecolor
		...
		sHTML = objColor1.drawColor +
			...
			"</tr></table>";
		sHTML = boxDrawPop(390,"Letterkleur",sHTML);
	...
	if(this.usePageProperties && !document.getElementById("boxPage"))//boxPage
		{
		document.write("<iframe id=boxPage name=boxPage src='library/ACE/include/box_Page.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useTable && !document.getElementById("boxTable2"))//boxTable
		{
		document.write("<iframe id=boxTable2 name=boxTable2 src='library/ACE/include/box_Table.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useSymbol && !document.getElementById("boxSymbol"))//boxSymbol
		{
		document.write("<iframe id=boxSymbol name=boxSymbol src='library/ACE/include/box_Symbol.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}
	}
...
function ACEditor(oName)
	...
	this.ImagePageURL = "default_Image.asp";
	...
	this.InternalLinkPageHeight = 740;
