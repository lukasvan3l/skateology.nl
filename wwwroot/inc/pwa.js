...
function picasaweb(j){ //returns the list of all albums for the user
 ...
 $("<div style='margin-left:3px'>Fotoalbums</div><br>");
 $("<table border=0><tr>");
 ...
 // then display them in a table with 4 columns (along with the album title)
  ...
  $("<td valign=top><a class='standard' href='?a=fotos&albumid="+id_base+"'><img src='"+img_base+"?imgmax=160&crop=1' class='pwimages' /></a>");
  $("<br><table border=0><tr><td></td></tr></table><a class='standard' href='?a=fotos&albumid="+id_base+"'>"+ j.feed.entry[i].title.$t +"</a></td>");
...
function albums(j){  //returns all photos in a specific album
 ...
 $("<div style='margin-left:3px'><a class='standard' href='" + window.location.protocol + "//" + window.location.hostname+window.location.pathname+"?a=fotos'>Fotoalbums</a> &gt; "+ j.feed.title.$t +"&nbsp;&nbsp;["+np+" item"+item_plural+"]</div><br>");
 $("<table border=0><tr>");
 ...
 for(i=0;i<j.feed.entry.length;i++){
  ...
  var link_url = "?a=fotos&albumid="+_GET['albumid']+"&photoid="+id_base; //+"&photoids="+photoids;
  ...
  //$("<p><SPAN STYLE='font-size: 9px'>"+j.feed.entry[i].media$group.media$description.$t+"</span>");
  $("</td>");
...
function photo(j){//returns exactly one photo
 ...
 var album_base_path = window.location.protocol + "//" + window.location.hostname+window.location.pathname +"?a=fotos&albumid="+ _GET['albumid'];
...
var current_index_text = item_label_caps + " " + current_index + " van " + my_numpics;
if (is_video == 1) { current_index_text = current_index_text + "&nbsp;&nbsp;[VIDEO]"; }  //show in the breadcrumbs that the item is a video
$("<div style='margin-left:3px'><a class='standard' href='"+ window.location.protocol + "//" + window.location.hostname+window.location.pathname+"?a=fotos'>Fotoalbums</a> &gt; <a class='standard' href='" + album_base_path + "'>" + my_fixed_galleryname + "</a> &gt; <!--" + img_filename + "-->" + current_index_text + "</div><br>");
...
if (n1 == null) //we're at the last picture in the album; going forward takes us to the album index
 ...
 if (photo_array.length > 1) { $("<td><a class='standard' href='"+prev+"'>Vorige</a> </td><td></td>"); }
 $("<td> <a class='standard' href='"+album_base_path+"'>Terug naar album</a> </td>");
 if (photo_array.length > 1) { $("<td></td><td> <a class='standard' href='"+next+"'>Volgende</a></td>"); }
 $("</tr></table></center><br>");
 ...
 $("<center><a border=0 target=PICASA href='"+photo_link+"'><img id='picture' src='"+img_base+"?imgmax="+photosize+"' class='pwimages' /></a></center>");
 $("<br><div style='margin-left:2px'>"+j.entry.media$group.media$description.$t+"</div></p>");
